-- this file is used by Glyph to setup the Lua environment when starting
VERSION = "0.1.0"
MOD_VERSION = "3"

SCALE = tonumber(os.getenv("GLYPH_SCALE") or os.getenv("GDK_SCALE") or os.getenv("QT_SCALE_FACTOR")) or 1
...
if MACOS_RESOURCES then
  ...
  DATADIR = prefix and (prefix .. PATHSEP .. 'share' .. PATHSEP .. 'glyph') or (EXEDIR .. PATHSEP .. 'data')
end
USERDIR = (system.get_file_info(EXEDIR .. PATHSEP .. 'user') and (EXEDIR .. PATHSEP .. 'user'))
       or os.getenv("GLYPH_USERDIR")
       or ((os.getenv("XDG_CONFIG_HOME") and os.getenv("XDG_CONFIG_HOME") .. PATHSEP .. "glyph"))
       or (HOME and (HOME .. PATHSEP .. '.config' .. PATHSEP .. 'glyph'))
