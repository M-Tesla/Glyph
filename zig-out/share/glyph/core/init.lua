...
local function show_max_files_warning(dir)
  local message = dir.slow_filesystem and
    ...
    config.max_project_files.." files. Consider adding folders to ignore_files in config."
  core.warn(message)
end
...
--
-- DATADIR is the location of the installed Glyph Lua code, default color
-- schemes and fonts.
-- USERDIR is the location of the Glyph configuration directory.
--
...
function core.configure_borderless_window()
  ...
  core.zoom_bar:configure_hit_test(config.borderless)
  core.zoom_bar.visible = true  -- ZoomBar is always visible
end
...
function core.init()
  ...
  cur_node:split("up", core.zoom_bar, {y = true})
  cur_node = cur_node.b
  ...
  if not project_dir_abs then
    system.show_fatal_error("Error", "Please restart Glyph with a readable project directory.")
    os.exit(1)
  end
  ...
  if #plugins_refuse_list.userdir.plugins > 0 or #plugins_refuse_list.datadir.plugins > 0 then
    ...
    core.nag_view:show(
      ...
      string.format(
        ...
        "Please download a recent version compatible with mod-version:3.",
        table.concat(msg, ".\n\n")),
      opt, function(item)
...
function core.compose_window_title(title)
  return (title == "" or title == nil) and "Glyph" or title .. " - Glyph"
end
