...
if not widget_found then
  core.error("Widget library not found: install it under libraries/widget")
  return false
end
...
function Settings:setup_about()
  ---@type widget.label
  local title = Label(self.about, "Glyph")
  title.font = "big_font"
  ---@type widget.label
  ...
  local description = Label(
    self.about,
    "A lightweight code editor with a Zig backend and Lua frontend."
  )
  ...
  button:set_tooltip("Open https://github.com/")
  function button:on_click() open_link("https://github.com/") end
...
local contributors_list = {
  { "Rxi", "Lite original author", "https://github.com/rxi" },
  { "Francesco Abbate", "Lite XL original author", "https://github.com/franko" },
  ...
  { "Tesla", "Glyph original author & Glyph Zig backend author", "https://github.com/M-Tesla" }
}
...
-- required on user module, or priority tag is obeyed by Glyph.
--------------------------------------------------------------------------------
